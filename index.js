...
function getUserStatus(age) {
  ...
  } else if (age > 18 && age < 65) return age, `Дорослий`;
  else if (age >= 65) return age, `Пенсіонер`;
...
const memoizedArea = (prevSide = null, prevResult) => {
  return (side, cah = "Fetching from cache", cah2 = "Calculating result") => {
    if (prevSide === side || cah === prevSide) {
      console.log(`${side * side}${cah}`);
      return prevResult;
    }
    prevResult = `${side * side}${cah2}`;
    prevSide = side;
    return prevResult;
  };
};
...
let createCompose = function compose(n) {
